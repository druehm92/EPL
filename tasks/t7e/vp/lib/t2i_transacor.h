...
SC_MODULE(t2i_transacor)
{
	...
	// TLM Port:
	tlm_utils::simple_initiator_socket<t2i_transacor,32> initiatorSocket;
	tlm_utils::simple_target_socket<t2i_transacor,32> debugTargetSocket;	
	...
	// Constructor:
	SC_CTOR(t2i_transacor) : currentState("t2i_state"), clk("t2i_clk"), rst("t2i_rst"), dataout("t2i_dataout"),
	                         done("t2i_done"), datain("t2i_datain"), enable("t2i_enable"), addr("t2i_addr"), write("t2i_write")
	{
		transactionFinished = false;

		debugTargetSocket.register_transport_dbg(this,&t2i_transacor::transport_dbg);		
...
unsigned int t2i_transacor::transport_dbg(tlm::tlm_generic_payload& trans)
{
	return initiatorSocket->transport_dbg(trans);
}

void t2i_transacor::sendThread()
{
	while(true)
	{
