...
SC_MODULE(i2t_transacor)
{
	...
	// TLM Port:
	tlm_utils::simple_target_socket<i2t_transacor,32> targetSocket;
	tlm_utils::simple_initiator_socket<i2t_transacor,32> debugInitiatorSocket;
	...
	// Constructor:
	SC_CTOR(i2t_transacor) : currentState("i2t_state"), clk("i2t_clk"), rst("i2t_rst"), dataout("i2t_dataout"),
				 ...
				 write("i2t_write"), targetSocket("targetSocket")
	{
		...
		targetSocket.register_b_transport(this,&i2t_transacor::transport);
		targetSocket.register_transport_dbg(this,&i2t_transacor::transport_dbg);
...
// Methods
unsigned int i2t_transacor::transport_dbg(tlm::tlm_generic_payload& trans)
{
	return debugInitiatorSocket->transport_dbg(trans);
}

void i2t_transacor::transport(tlm::tlm_generic_payload& trans, sc_core::sc_time& delay)
{
	#if SDEBUG 
